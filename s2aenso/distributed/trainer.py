''' Trainer for distributed training with PyTorch. 

@Author  :   Sebastian Hofmann, Jannik Thuemmel 
@Time    :   2024/02/06 16:23:09
...
class DistributedTrainer(TrainerInterface):  
    """ The following methods are executed in the following order:
        1. reset()
        3. Run train() to start training: 
            a. setup()
                i. setup_process_group()
                ii. setup_checkpointing()
                iii. setup_dataset()
                iv. setup_model()
                v. setup_training()
                vi. setup_wandb()
    ...
    def reset(self):
        ...
        self.train_metrics_step = MetricSaver(epochs=332)
        self.val_metrics_step = MetricSaver(epochs=83)
    ...
    def train(self):
        ...
        while self.current_epoch <= self.cfg.epochs:
            self.pre_epoch()
            print("Pre-epoch completed")
            self.train_epoch()
            print("Train-epoch completed")
            self.evaluate_epoch()
            print("Evaluate-epoch completed")
            self.post_epoch()
            print("Post-epoch completed")
        self.post_training()
        print("Post-training completed")
        
    def load_state_dict(self, state_dict):
        self.current_epoch = state_dict['epoch']
        self.train_metrics_step = MetricSaver(state_dict['train_metrics_step'])
        self.val_metrics_step = MetricSaver(state_dict['val_metrics_step'])
    ...
    def state_dict(self):
        state_dict = {
            ...
            'train_metrics_step': self.train_metrics_step.epochs,
            'val_metrics_step': self.val_metrics_step.epochs,
    ...
    def log_wandb_step(self):
        metrics = {}
        for key, value in self.train_metrics_step.scalar_metrics()[-1].items():
            metrics[f'train_step/{key}'] = value

        for key, value in self.val_metrics_step.scalar_metrics()[-1].items():
            metrics[f'val_step/{key}'] = value

        for key, value in self.misc_metrics_step.scalar_metrics()[-1].items():
            metrics[f'misc_step/{key}'] = value
