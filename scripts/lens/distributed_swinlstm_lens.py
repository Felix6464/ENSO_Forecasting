...
class LENSTrainer(trainer.DistributedTrainer):
    ...
    def create_dataset(self):
        ...
        # CESM2
        self.train_data = data.load_cesm_lens(root_dir = self.cfg.data_dir, 
        ...
        val_data = data.load_cesm_lens(root_dir = self.cfg.data_dir, 
                                    vars = self.cfg.in_vars, 
                                    sequence_length= self.cfg.sequence_length,
                                    split= 'val', 
                                    rank = self.rank,
        ...
        self.val2_dl = torch.utils.data.DataLoader(val2_data, 
                                          ...
                                          pin_memory=True,
                                          num_workers=self.cfg.num_workers)
        print("LENGTH TRAIN; VAL; VAL2: ", len(train_dl), len(val_dl), len(self.val2_dl))
    ...
    def create_scheduler(self, lr):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer=self.optimizer, T_max=self.cfg.epochs, eta_min=self.cfg.min_lr,
        )
        return scheduler
    ...
    def forward_step(self, batch_idx, batch, **kwargs):
        ...
        idx_out_vars = [self.train_data.datasets[0].variable_mapping[var] for var in self.cfg.out_vars]
        y = y[:, idx_out_vars]
...
def main():
    ...
    params = {
        "name": f"base_swinlstm_lens_{slurm_id}",
        "data_dir" : PATH + '/../../data/processed_data/cesm2_lens/historical/',
        "oras5_dir" : PATH + '/../../data/processed_data/enso_data_pacific/oras5/',
