...
rename_vars = {'pottmp': f'temp_ocn_{config["level"]}'}
#seven-layer ocean temperature anomalies in the upper 150 m (at 5, 20, 40, 60, 90, 120, and 150 m)
# thetao
# %%
# Load and merge data
# ======================================================================================
...
orig_da = orig_da.transpose('time', 'level', 'lat', 'lon')

if config["level"] is not None:
    orig_da = orig_da.isel(level=config["level"])
...
n_time, n_lat, n_lon = orig_da.shape         #12, 360, 418
orig_data_flat = orig_da.values.reshape((n_time, -1))
#orig_lats = orig_da['lat'].values.ravel()
#orig_lons = orig_da['lon'].values.ravel()
