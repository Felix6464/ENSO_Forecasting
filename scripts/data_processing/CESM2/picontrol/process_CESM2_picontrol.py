...
if False:
    config = dict(
        dirpath="/mnt/qb/goswami/data/cmip6_lens/month/piControl/CESM2/sst/",
...
orig_da = orig_ds[var]
# TODO: For testing
#orig_da = orig_da.isel(time=slice(0,100)) 
...
# Save climatology
climatology.to_dataset().to_netcdf(
    os.path.join(config['outpath'], config['prefix'])  
    + f"climatology_{interp_da.name}"
    + f"_lat{'_'.join(map(str, config['lat_range']))}"
    + f"_lon{'_'.join(map(str,config['lon_range']))}"
    + f"_gr{config['grid_step']}.nc"
)
