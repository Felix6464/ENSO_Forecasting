...
def main():
    ...
    parser.add_argument('-r', '--repository', default="/mnt/qb/goswami/data/cmip6_lens/month/piControl/CESM2/tauy/", help='Path to the repository containing the files')
    ...
    for filename in os.listdir(args.repository):
        ...
        if match:
            ...
            name = f'preprocess_tauy_{filename}'
            
            # Build the command dynamically
            command = f'python {PATH}/process_CESM2_picontrol_tauy.py'
            command += f' -path {args.repository}'
            command += f' -prefix {filename}'  # Use the filename as the prefix
            command += f' -name CMIP6-piControl.TAUY.{year_range}'
            command += f' -var TAUY'
            command += f' -cpus 8'
            command += f' -outpath /mnt/qb/datasets/STAGING/goswami/CMIP6_LENS/CESM2/piControl/tauy_1_2_grid/'
