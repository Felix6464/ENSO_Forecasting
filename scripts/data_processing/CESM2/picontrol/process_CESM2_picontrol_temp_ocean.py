...
for filename in os.listdir(config['dirpath']):
    ...
    var = config['var']
    print(os.path.join(config['dirpath'], filename) + "*.nc", flush=True)

    orig_ds = xr.open_mfdataset(os.path.join(config['dirpath'], filename) + "*.nc", 
                                chunks={'time': 100},
                                combine='by_coords')
    orig_da = orig_ds[var]
    # TODO: For testing
    ...
    outpath = outpath + f'year_{year_range}/'

    if not os.path.exists(outpath):
        print(f"Create directoty {outpath}", flush=True)
        os.makedirs(outpath)
    varname = da_anomaly.name
    name = config['name'] + f'{year_range}'

    outfname =(os.path.join(outpath, name)  
