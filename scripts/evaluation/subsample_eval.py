...
for model_num_ in models_swin:
    ...
    pred_path = PATH + f"preds_xr_{model}_{model_num}.zarr"
    targ_path = PATH + f"targets_xr_{model}_{model_num}.zarr"
    ...
    val_ds_adapt = xr.open_dataset(PATH + "/../../../../results/val_ds_zhou.nc")
    common_lsm = xr.open_dataset(PATH + '/../../../../../../scripts/data_processing/lsm_zhou.nc')['lsm'].data
    ...
    ds_sst = xr.open_dataset("C:/Users/felix/PycharmProjects/deeps2a-enso/data/paper_data/godas_val.nc")

    ds_sst_level0 = ds_sst['temperatureNor']

    # Calculate climatology -> data already in anomalies
    ds_sst_level0 = ds_sst_level0.sel(n_mon=val_range)
    ...
    rmse_sst_level0_spatial = np.sqrt(((ds_sst_level0**2).mean('n_mon')))
    rmse_sst_level0 = np.sqrt((ds_sst_level0**2).mean(dim=['n_mon', 'lat', 'lon']))
    rmse_sst_level0_nino3 = np.sqrt((ds_sst_level0_nino3**2).mean(dim=['n_mon', 'lat', 'lon']))
    rmse_sst_level0_nino34 = np.sqrt((ds_sst_level0_nino34**2).mean(dim=['n_mon', 'lat', 'lon']))
    rmse_sst_level0_nino4 = np.sqrt((ds_sst_level0_nino4**2).mean(dim=['n_mon', 'lat', 'lon']))
    ...
    lags = [3, 12, 20]
    val_ds_adapt = val_ds_adapt.sel(lat=slice(-19, 18), lon=slice(90, 329))
        ...
        for lag in lags:
            ...
            for key, x in dict(Prediction=pred.unsqueeze(1), Target=targ.unsqueeze(1)).items():
                ...
                for i, var in enumerate(val_ds_adapt.data_vars):
                    da = xr.DataArray(
                        data= x[0, i, lag - 1],
                        coords=val_ds_adapt.isel(lev=1).coords,
                        name = var)
            ...
            for ax, key, title in zip(axs, keys, titles):
                ...
                for var in x.data_vars:
                    im = ax.pcolormesh(x.lon, x.lat, x[var], cmap=cmap, vmin=vmin, vmax=vmax, transform=ccrs.PlateCarree())
                    ax.coastlines()
                    ...
                    # Add contour lines
                    ax.contour(x.lon, x.lat, x[var], levels=2, colors='k', linewidths=0.3, transform=ccrs.PlateCarree())
    ...
    for subsample_key, subsample_results in loss_dict.items():
        ...
        loss_spatial = subsample_results['rmse_orig'][:, 0:48, :]  # Use the RMSE values for this subsample
        loss_climatology_spatial = rmse_sst_level0_spatial.sel(lat=slice(-19, 18), lon=slice(90, 329))  # Climatology data for skill score calculation
        ...
        loss_spatial = subsample_results['nino_rmse_3'][:, 0:48, :]
        val_ds_3i = val_ds_adapt.sel(lat=slice(-4, 6), lon=slice(210, 269))
        loss_climatology_3i_spatial = rmse_sst_level0_spatial.sel(lat=slice(-4, 6), lon=slice(210, 269))
        ...
        loss_spatial = subsample_results['nino_rmse_34'][:, 0:48, :]
        val_ds_34i = val_ds_adapt.sel(lat=slice(-4, 6), lon=slice(190, 239))
        loss_climatology_34i_spatial = rmse_sst_level0_spatial.sel(lat=slice(-4, 6), lon=slice(190, 239))
        ...
        loss_spatial = subsample_results['nino_rmse_4'][:, 0:48, :]
        val_ds_4i = val_ds_adapt.sel(lat=slice(-4, 6), lon=slice(174, 223))
        loss_climatology_4i_spatial = rmse_sst_level0_spatial.sel(lat=slice(-4, 6), lon=slice(174, 223))
    ...
    if three_month_mean:
        ...
        new_dataarray = xr.DataArray(
            ...
            dims=target_slice.sizes,  # Same dimensions as target_slice
