...
model_num = 815035
run_specification = "1_2g_best_allV"
lens = True
...
for dataset in datasets:
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_dl):
            ...
            vars_ = range(pred.shape[2])  # 9 variables
            latitude = range(pred.shape[3])    # 52 latitudes
            longitude = range(pred.shape[4])   # 132 longitudes
            ...
            #calculate loss
            mse_ = l2(pred, targ)
            mse_ *= (1 - lsm)[None, None, None, :, :]

            mse += mse_.sum(dim=(0, 2, 3, 4)).div(N)
            rmse += torch.sqrt(mse_.sum(dim=(0, 2, 3, 4)).div(N))
            ...
            nino_rmse_3_l += torch.sqrt(l2(nino_pred_3, nino_true_3).mean(dim=(0, 2, 3, 4)))
            ...
            nino_rmse_34_l += torch.sqrt(l2(nino_pred_34, nino_true_34).mean(dim=(0, 2, 3, 4)))
            ...
            nino_rmse_4_l += torch.sqrt(l2(nino_pred_4, nino_true_4).mean(dim=(0, 2, 3, 4)))
