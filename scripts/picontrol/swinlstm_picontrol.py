''' Train a SwinLSTM model on the CESM2-LENS piControl dataset with single GPU. 

@Author  :   Jakob Schlör 
@Time    :   2024/03/11 10:31:25
...
cfg = {
    "name": f"swinlstm_piControl_{slurm_id}",
...
for current_epoch in range(num_epochs):
    ...
    model.eval()
    mode = 'val'
    print("Start validation loop", flush=True)
    if cfg["nino_loss"]:
    ...
    print(f'Epoch: {current_epoch}, Train Loss: {epoch_tracker["train/loss_avg"]:,.2e}, '
          ...
          + f'Val RMSE: {epoch_tracker["val/rmse_avg"]:,.2e}'
          #+ f'Val RMSE - SSH: {epoch_tracker["val/rmse_ssh"]:,.2e}'
          #+ f'Val RMSE - SST: {epoch_tracker["val/rmse_sst"]:,.2e}'
          #+ f'Val CRPS - SSH: {epoch_tracker["val/crps_ssh"]:,.2e},'
          #+ f'Val CRPS - SSH: {epoch_tracker["val/crps_sst"]:,.2e},'
          , flush=True)
